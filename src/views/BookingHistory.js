...
const BookingHistory = (props) => {
  ...
  const [bookingRowData, setBookingRowData] = useState(null);


  const handleBookingAlertClose = () => {
    setBookingRowData(null)
  };
  ...
  return (
    ...
    <MaterialTable
      ...
      onRowClick={((evt, selectedRow) => {
        ...
        setBookingRowData({booking_id:selectedRow.id,mainData:selectedRow})
      })}
      options={{
    ...
    {selectedBooking && selectedBooking.paymentPacket  && (selectedBooking.status ==='PENDING' || (selectedBooking && selectedBooking.selectedBid && selectedBooking.status ==='NEW')) && role==='customer'?
      <Modal
        disablePortal
        disableEnforceFocus
        disableAutoFocus
            ...
            <Grid item xs={12} sm={12} md={12} lg={12}>
              <FormControl component="fieldset">
                ...
                <RadioGroup name="selectedMethod" value={selectedMethod} onChange={handleChange}>
                  <FormControlLabel key={"wallet"} value={0} control={<Radio />} label={t('use_wallet_balance') + " " + (settings.swipe_symbol===false? settings.symbol + auth.profile.walletBalance :  auth.profile.walletBalance + settings.symbol) + ")"} /> 
    ...
    {users && data && rowIndex>=0?
      ...
      <Modal
        ...
        disableEnforceFocus
        disableAutoFocus
        open={bookingRowData ? true : false}
        onClose={handleBookingAlertClose}
        className={classes.modal}
      >
        <Grid container spacing={1} className={classes.paper}>
          <BookingDetails bookingData={bookingRowData} />
          <Grid
            sx={{
              display: "flex",
              flexDirection: "row",
